...
class NoiseDetector:
    ...
    def detect_noise(self, X: np.ndarray) -> NoiseProfile:
        ...
        noise_types = self._classify_noise_types(
            X_scaled, combined_scores, method_results
        )
    ...
    def _detect_statistical_outliers(
        ...
        mahalanobis_dist = np.sqrt(
            np.sum(np.dot(X - mean, inv_cov) * (X - mean), axis=1)
        )

        # Combine evidence
        max_z_scores = np.max(np.abs(z_scores), axis=1)
        noise_scores = 0.7 * stats.norm.cdf(max_z_scores) + 0.3 * stats.chi2.cdf(
            mahalanobis_dist, df=X.shape[1]
        )
    ...
    def _generate_recommendations(
        ...
        if "DBSCAN" in recommendations["algorithm"][0]:
            ...
            param_recommendations["min_samples"] = max(
                2, int(np.log(len(X)) * (1 - noise_ratio))
            )
